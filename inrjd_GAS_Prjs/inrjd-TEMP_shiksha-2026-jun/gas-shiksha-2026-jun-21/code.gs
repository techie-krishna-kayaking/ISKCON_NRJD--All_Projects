...
function _coerceSheetToPlainText(sheet, lastCol) {
  // Force all used cells to text and rewrite displayed values to prevent type inference.
  var lastRow = sheet.getLastRow();
  if (lastRow < 1 || lastCol < 1) return;
  var rng = sheet.getRange(1, 1, lastRow, lastCol);
  var displayValues = rng.getDisplayValues();
  rng.setNumberFormat('@');
  rng.setValues(displayValues);
}
...
function _extractLast4Aadhar(aadharRaw) {
  var aadhar = (aadharRaw || '').toString().replace(/\D/g, '');
  if (!aadhar || aadhar.length < 4) return '';
  return aadhar.slice(-4);
}

function _extractDdMm(dobRaw) {
  var dob = (dobRaw || '').toString().trim();
  if (!dob) return '';

  // Expected from form input type=date: yyyy-mm-dd
  var iso = dob.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (iso) return iso[3] + iso[2];

  // Also accept dd/mm/yyyy or dd-mm-yyyy
  var dmy = dob.match(/^(\d{2})[\/-](\d{2})[\/-](\d{4})$/);
...
function generateShikshaCode(bioformData) {
  ...
  var aadhar = (bioformData.aadhar || "").toString().trim();
  var dob = (bioformData.dob || "").toString().trim();

  if (!aadhar || !dob) return "ERROR_MISSING_FIELDS";

  var ddmm = _extractDdMm(dob);
  var last4 = _extractLast4Aadhar(aadhar);

  if (!ddmm) return "ERROR_INVALID_DOB";
  if (!last4) return "ERROR_INVALID_AADHAR";

  return ddmm + last4; // DDMM1234
}
...
function submitBioForm(bioformData) {
  ...
  // Keep tab5 fully plain-text to avoid automatic data type interpretation.
  _coerceSheetToPlainText(sheet, lastCol);
  ...
  // Write as explicit text (no numeric/date auto-casting).
  newRow = newRow.map(function(v) { return (v === null || typeof v === 'undefined') ? '' : v.toString(); });
...
function submitShikshaData(formData) {
  ...
  // Append to tab5 as text-only values (no numeric/date auto-casting)
  newRow = newRow.map(function(v) { return (v === null || typeof v === 'undefined') ? '' : v.toString(); });
  var targetRow = sheet.getLastRow() + 1;
  var targetRange = sheet.getRange(targetRow, 1, 1, lastCol);
  targetRange.setNumberFormat('@');
